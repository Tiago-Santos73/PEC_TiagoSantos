...
class Plate(torch.nn.Module):
    ...
    def define_domain(self):
            ...
            # Verificar se a chave existe em self.bc.new_sets (pontos de fronteira)
                ...
                # Preencher a terceira coluna com os valores de t (domínio de t)
                        BC_t = torch.empty(points.shape[0], 1).uniform_(
                            self.parameters['t domain'][0],
                            self.parameters['t domain'][1]
                        )
                        bc_tensor[:, 2] = BC_t[:, 0]  # Preencher o domínio de tempo

                # Preencher a quarta coluna com o valor da condição de fronteira
                        bc_value = self.bc.entry_values.get(key, self.parameters['T0'])  # Valor da BC
                        bc_tensor[:, 3] = float(bc_value)  # Preencher o valor da condição de fronteira
            ...
            print(self.xt)  # Concatenar ao longo da primeira dimensão (dim=0)
