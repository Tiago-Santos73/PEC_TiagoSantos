...
def BC():
    ...
    # Label da janela
    label_param = tk.Label(BC_janela, text="Condições de Fronteira", font=("Helvetica", 16, "bold"))
    ...
    def save_entry_values():
        ...
        # Criar a nova variável new_sets de acordo com a seleção de 1D, 2D ou 3D
        ...
        for key, value in sets.items():
            if dimension == 1:  # Para 1D, remove as duas últimas colunas
                new_sets[key] = value[:, :-2]
            elif dimension == 2:  # Para 2D, remove a última coluna
                new_sets[key] = value[:, :-1]
            else:  # Para 3D, mantém tudo como está
                new_sets[key] = value

        #print("new_sets criado com sucesso:\n", new_sets)
        #print(entry_values)
        #for key in sets.keys():
             #print(var_dict[key].get())
    ...
    # Frame para os botões de seleção de 1D, 2D e 3D
    dimension_frame = tk.Frame(BC_janela)
    dimension_frame.pack(pady=10)

    dimension_var = tk.IntVar(value=3)  # Valor padrão: 3D

    # Criando os RadioButtons para escolher a dimensão
    radio_1d = tk.Radiobutton(dimension_frame, text="1D", variable=dimension_var, value=1, font=("Helvetica", 12))
    radio_2d = tk.Radiobutton(dimension_frame, text="2D", variable=dimension_var, value=2, font=("Helvetica", 12))
    radio_3d = tk.Radiobutton(dimension_frame, text="3D", variable=dimension_var, value=3, font=("Helvetica", 12))

    radio_1d.pack(side='left', padx=5)
    radio_2d.pack(side='left', padx=5)
    radio_3d.pack(side='left', padx=5)
